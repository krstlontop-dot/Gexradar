...
def add_reference_lines(fig, spot, gflip, call_wall=None, put_wall=None, vol_trigger=None, max_pain=None):
    import plotly.graph_objects as go
    _tr = get_theme()

    # try to read x range from chart
    try:
    ...
    levels = [
        ("Spot", spot, _tr["spot_line"]),
        ("Gamma Flip", gflip, _tr["gamma_line"]),
        ("Call Wall", call_wall, _tr["green"]),
        ("Put Wall", put_wall, _tr["red"]),
        ("Vol Trigger", vol_trigger, _tr["amber"]),
        ("Max Pain", max_pain, _tr["violet"]),
    ]
    ...
    for label, level, color in levels:
        ...
        fig.add_trace(go.Scatter(
            ...
            hovertemplate=f"<b>{label}</b><br>Fiyat: {level:.2f}<extra></extra>",
            showlegend=False,
        ))
    ...
    line_specs = [
        (gflip, _tr["gamma_line"], "dot",   "  Gamma Flip", _tr["chart_t3"]),
        (spot,  _tr["spot_line"],  "dash",  "  Spot",       _tr["spot_line"]),
        (call_wall, _tr["green"],  "dash",  "  Call Wall",  _tr["green"]),
        (put_wall,  _tr["red"],    "dash",  "  Put Wall",   _tr["red"]),
        (vol_trigger, _tr["amber"],"dash",  "  Vol Trigger",_tr["amber"]),
    ]

    for level, color, dash, label, label_color in line_specs:
        if level is None:
            continue
        fig.add_hline(
            y=level,
...
def render_kl_panel(spot, gflip, cwall, pwall, mpain, vtrig, mwall, mval):
    ...
    rows = [
        ...
        (_tkl["violet"], "Max Pain",      f"${mpain:.2f}"),
...
def _render_daily_levels(ticker: str, spot: float, df, raw_df, T: dict):
    ...
    def _render_section(data: dict, section: str):
        ...
        if max_pain:
            cards += _level_card("Max Pain", f"${max_pain:.2f}", T["violet"], "Option pain target")
...
function initPrice() {
  const lvlShapes = [
    ...
    {type:'line',x0:0,x1:1,xref:'paper',y0:LEVELS.max_pain,  y1:LEVELS.max_pain, line:{color:TH.violet,width:1,dash:'dash'}},
  ...
  const lvlAnnot = [
    ...
    {x:1,xref:'paper',y:LEVELS.max_pain,  yref:'y',text:'  Max Pain $'+LEVELS.max_pain,   showarrow:false,xanchor:'left',font:{size:8,color:TH.violet, family:'JetBrains Mono'}},
  ];
...
function updateGEX() {
  ...
  Plotly.react(gexDiv, [{
    ...
    shapes:[
      ...
      {type:'line',x0:-maxX*1.5,x1:maxX*1.5,y0:LEVELS.call_wall,y1:LEVELS.call_wall,
       line:{color:TH.green,width:1,dash:'dot'}},
      {type:'line',x0:-maxX*1.5,x1:maxX*1.5,y0:LEVELS.put_wall,y1:LEVELS.put_wall,
       line:{color:TH.red,width:1,dash:'dot'}},
      {type:'line',x0:-maxX*1.5,x1:maxX*1.5,y0:LEVELS.vol_trigger,y1:LEVELS.vol_trigger,
       line:{color:TH.amber,width:1,dash:'dash'}},
      {type:'line',x0:-maxX*1.5,x1:maxX*1.5,y0:LEVELS.max_pain,y1:LEVELS.max_pain,
       line:{color:TH.violet,width:1,dash:'dash'}},
    ...
    annotations:[
      ...
      {x:maxX*1.35,y:LEVELS.call_wall,text:'Call Wall',showarrow:false,xanchor:'right',yanchor:'middle',
       font:{size:8,color:TH.green,family:'JetBrains Mono'},bgcolor:TH.bg1,borderpad:2},
      {x:maxX*1.35,y:LEVELS.put_wall,text:'Put Wall',showarrow:false,xanchor:'right',yanchor:'middle',
       font:{size:8,color:TH.red,family:'JetBrains Mono'},bgcolor:TH.bg1,borderpad:2},
      {x:maxX*1.35,y:LEVELS.gamma_flip,text:'Zero Γ',showarrow:false,xanchor:'right',yanchor:'middle',
       font:{size:8,color:TH.t2,family:'JetBrains Mono'},bgcolor:TH.bg1,borderpad:2},
      {x:maxX*1.35,y:LEVELS.vol_trigger,text:'Vol Trigger',showarrow:false,xanchor:'right',yanchor:'middle',
       font:{size:8,color:TH.amber,family:'JetBrains Mono'},bgcolor:TH.bg1,borderpad:2},
      {x:maxX*1.35,y:LEVELS.max_pain,text:'Max Pain',showarrow:false,xanchor:'right',yanchor:'middle',
       font:{size:8,color:TH.violet,family:'JetBrains Mono'},bgcolor:TH.bg1,borderpad:2},
...
def dashboard():
    ...
    if st.session_state.radar_mode == "GEX":
        ...
        with chart_col:
            ...
            add_reference_lines(fig, spot_price, gamma_flip, call_wall, put_wall, vol_trigger, max_pain)
